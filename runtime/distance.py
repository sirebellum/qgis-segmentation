...
def _prepare_reference_tensor(reference, device, compute_dtype):
    ...
    try:
        ref = torch.as_tensor(
            reference, device=device_obj, dtype=compute_dtype)
    except RuntimeError:
...
def _pairwise_dist2(chunk, reference, reference_norm):
    ...
    dist2 = dist2 - 2.0 * \
        torch.matmul(chunk, reference.transpose(0, 1)).float()
    dist2 = dist2.clamp_min_((0.0))
    if torch.isnan(dist2).any() or torch.isinf(dist2).any():
        raise FloatingPointError(
            "NaN or Inf detected in distance computation.")
    return dist2
...
def _argmin_distances_chunked(
    ...
    ref, ref_norm, device_obj, dtype = _prepare_reference_tensor(
        reference, device, compute_dtype)
    ...
    try:
        for start in range(0, total, step):
            ...
            chunk = torch.as_tensor(
                data_np[start:end], device=device_obj, dtype=dtype)
            dist2 = _pairwise_dist2(chunk, ref, ref_norm)
            labels[start:end] = torch.argmin(
                dist2, dim=1).cpu().numpy().astype(np.int64)
...
def _topk_distances_chunked(
    ...
    try:
        for start in range(0, total, step):
            ...
            best_dist = torch.full((rows, cur_k), float(
                "inf"), device=device_obj, dtype=torch.float32)
            best_idx = torch.full(
                (rows, cur_k), -1, device=device_obj, dtype=torch.int64)
            ...
            while ref_start < ref.shape[0]:
                ...
                dist2 = _pairwise_dist2(
                    chunk, ref_block, ref_norm_block).to(torch.float32)
                offset = torch.arange(ref_start, ref_end,
                                      device=device_obj, dtype=torch.int64)
                ...
                best_dist, top_idx = torch.topk(
                    merged_dist, cur_k, largest=False)
