...
- **Runtime behavior**: CNN path tiles (default 512px, slider-tuned 192–768px), computes adaptive chunk plans, aggregates chunk scores, applies optional post-smoothing, and stitches; K-Means clusters pooled descriptors via the torch path and upsamples. Cancellation via `CancellationToken`; progress parsed from worker status strings; texture remap is available but disabled by default in the plugin.
