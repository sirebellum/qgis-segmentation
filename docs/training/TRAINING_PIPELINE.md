...
- Loader: `training/data/dataset.py` builds two augmented RGB views per sample (flip/rotate/noise) with identity warp grids; elevation channels are not supported. A GeoTIFF path is available via `training/data/geo_patch_dataset.py` for RGB patches; patch sizes are configured via `data.patch_sizes` (default [256, 512, 1024]) and sampled per step (uniform by default). Synthetic data remains the smoke fallback.
