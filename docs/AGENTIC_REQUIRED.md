...
## Zip archive command
```bash
cd docs && zip agent.zip ../README.md CODE_DESCRIPTION.md CODESTATE.md AGENTIC_HISTORY.md AGENTIC_REQUIRED.md plugin/ARCHITECTURE.md plugin/MODEL_NOTES.md plugin/RUNTIME_STATUS.md
```
...
- **Plugin tests** (tests/): 100 passing, 4 skipped (QGIS/GPU gated)
...
.venv/bin/flake8 *.py runtime/ --max-line-length=120 --ignore=E501,W503 --exclude=resources.py
```
